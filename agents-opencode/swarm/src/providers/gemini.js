...
export class GeminiAdapter extends ProviderAdapter {
  constructor(options = {}) {
    const command = process.env.SWARM_GEMINI_PATH || "gemini";
    const timeoutMs = process.env.SWARM_GEMINI_TIMEOUT_MS
      ? Number(process.env.SWARM_GEMINI_TIMEOUT_MS)
      ...
      child.on("close", (exitCode, signal = null) => {
        ...
        if (timedOut) {
          resolve({
            ...
            exitCode,
            signal,
            error: `Gemini CLI timed out after ${this.timeoutMs}ms`
          });
          return;
        }
